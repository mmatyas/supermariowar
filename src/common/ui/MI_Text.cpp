...
void MI_Text::Draw()
{
    ...
    if (m_align == TextAlign::LEFT && iw == 0)
        font->draw(m_pos.x, m_pos.y, szText);
    else if (m_align == TextAlign::LEFT)
        font->drawChopRight(m_pos.x, m_pos.y, iw, szText);
    else if (m_align == TextAlign::CENTER)
        font->drawCentered(m_pos.x, m_pos.y, szText);
    else if (m_align == TextAlign::RIGHT)
        font->drawRightJustified(m_pos.x, m_pos.y, szText);
}
