...
MapList::MapList(bool fWorldEditor)
{
    ...
    while (d(curname)) {
        ...
        maps.insert(std::pair<std::string, MapListNode*>(stripCreatorAndDotMap(curname), node));
    }
    ...
    while (tourMapDir(curname)) {
        MapListNode * node = new MapListNode(tourMapDir.fullName(curname));
        worldmaps.insert(std::pair<std::string, MapListNode*>(stripCreatorAndDotMap(curname), node));
    }
...
void MapList::add(const char * name)
{
    ...
    maps.insert(std::pair<std::string, MapListNode*>(stripCreatorAndDotMap(name), node));
}
