...
MovingPlatformPath* MapReader1700::read_platform_path_details(BinaryFile& mapfile, short iPathType, bool fPreview)
{
    if (iPathType == 0) { //segment path
        ...
        return new StraightPath(speed, {startX, startY}, {endX, endY}, fPreview);
    ...
    if (iPathType == 1) { //continuous path
        ...
        return new StraightPathContinuous(speed, {startX, startY}, angle, fPreview);
    ...
    if (iPathType == 2) { //elliptical path
        ...
        return new EllipsePath(speed, angle, {radiusX, radiusY}, {centerX, centerY}, fPreview);
