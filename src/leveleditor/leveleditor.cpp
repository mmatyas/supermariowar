                    ...
                    if (event.button.button == SDL_BUTTON_LEFT && !ignoreclick) {
                        if (edit_mode == 0) {  // paint selected blocks
                            ...
                            if (g_map->mapitems.size() < MAXMAPITEMS) {
                                ...
                                if (g_map->mapdatatop[iClickX][iClickY] != TileType::NonSolid && g_map->mapdatatop[iClickX][iClickY] != TileType::SolidOnTop && g_map->mapdatatop[iClickX][iClickY] != TileType::IceOnTop) {
                                    fTileNotAvailable = true;
                                }
...
{
    short iTilesetIndex = iBlockSize == TILESIZE ? 0 : iBlockSize == PREVIEWTILESIZE ? 1
                                                                                     : 2;
    ...
    for (short i = 0; i < MAPWIDTH; i++) {
        for (short j = 0; j < MAPHEIGHT; j++) {
            ...
            if ((move_mode == 1 || move_mode == 3) && i - move_offset_x >= 0 && i - move_offset_x < MAPWIDTH && j - move_offset_y >= 0 && j - move_offset_y < MAPHEIGHT && selectedtiles[i - move_offset_x][j - move_offset_y]) {
                if (fUseCopied)
...
void drawmap(bool fScreenshot, short iBlockSize, bool fWithPlatforms)
{
    ...
    if ((viewblocks && !view_only_layer) || fScreenshot) {
        short iTilesizeIndex = iBlockSize == 32 ? 0 : iBlockSize == 16 ? 1
                                                                       : 2;
    ...
    if (!view_only_layer || fScreenshot) {
        for (const MapItem& item : g_map->mapitems) {
            rm->spr_mapitems[iBlockSize == TILESIZE ? 0 : iBlockSize == PREVIEWTILESIZE ? 1
                                                                                        : 2]
                .draw(item.ix * iBlockSize, item.iy * iBlockSize, item.itype * iBlockSize, 0, iBlockSize, iBlockSize);
        }
        ...
        if (!fScreenshot) {
            for (int j = 0; j < MAPHEIGHT; j++) {
                for (int i = 0; i < MAPWIDTH; i++) {
                    ...
                    if ((move_mode == 1 || move_mode == 3) && iNewX >= 0 && iNewX < MAPWIDTH && iNewY >= 0 && iNewY < MAPHEIGHT && selectedtiles[iNewX][iNewY]) {
        ...
        for (int j = 0; j < MAPHEIGHT; j++) {
            for (int i = 0; i < MAPWIDTH; i++) {
                ...
                if (warp->connection != -1) {
                    ...
                    SDL_BlitSurface(rm->spr_warps[iBlockSize == TILESIZE ? 0 : iBlockSize == PREVIEWTILESIZE ? 1
                                                                                                             : 2]
                                        .getSurface(),
                        &rSrc, screen, &rDst);
                }
                ...
                if (event.button.button == SDL_BUTTON_LEFT) {
                    ...
                    for (short iLayer = 0; iLayer < 3; iLayer++) {
                        for (short k = 0; k < NUM_EYECANDY; k++) {
                            if (event.button.x >= ix && event.button.x < ix + 90 && event.button.y >= k * 65 + 20 && event.button.y < k * 65 + 72) {
                                short mask = 1 << k;
                ...
                case SDL_MOUSEBUTTONDOWN: {
                    ...
                    if (iBlockType == 1 || iBlockType == 15) {
                        ...
                        if (piSetting) {
                            ...
                            if ((event.button.button == SDL_BUTTON_RIGHT && (iMouseState & SDL_BUTTON_LMASK)) || (event.button.button == SDL_BUTTON_LEFT && (iMouseState & SDL_BUTTON_RMASK))) {
...
enum {
    PLATFORM_EDIT_STATE_SELECT,
    PLATFORM_EDIT_STATE_PATH_TYPE,
    PLATFORM_EDIT_STATE_CHANGE_PATH_TYPE,
    PLATFORM_EDIT_STATE_EDIT,
    PLATFORM_EDIT_STATE_PATH,
    PLATFORM_EDIT_STATE_TEST,
    PLATFORM_EDIT_STATE_TILETYPE,
    PLATFORM_EDIT_STATE_ANIMATED,
    PLATFORM_EDIT_STATE_MOVE
};
                ...
                if (event.button.button == SDL_BUTTON_LEFT && !ignoreclick) {
                    if (PLATFORM_EDIT_STATE_SELECT == iPlatformEditState || PLATFORM_EDIT_STATE_MOVE == iPlatformEditState) {
                        ...
                        for (int iPlatform = 0; iPlatform < g_iNumPlatforms; iPlatform++) {
                            if (event.button.x >= g_Platforms[iPlatform].rIcon[1].x && event.button.x < g_Platforms[iPlatform].rIcon[1].x + g_Platforms[iPlatform].rIcon[1].w && event.button.y >= g_Platforms[iPlatform].rIcon[1].y && event.button.y < g_Platforms[iPlatform].rIcon[1].y + g_Platforms[iPlatform].rIcon[1].h) {
                                if (PLATFORM_EDIT_STATE_SELECT == iPlatformEditState) {
                        ...
                        if (PLATFORM_EDIT_STATE_SELECT == iPlatformEditState && g_iNumPlatforms < MAX_PLATFORMS && event.button.x >= rNewButton[1].x && event.button.x < rNewButton[1].x + rNewButton[1].w && event.button.y >= rNewButton[1].y && event.button.y < rNewButton[1].y + rNewButton[1].h) {
                        ...
                        if (!ignoreclick) {
                            ...
                            for (int iType = 0; iType < 3; iType++) {
                                if (iClickX >= rTypeButton[iType][1].x && iClickX < rTypeButton[iType][1].x + rTypeButton[iType][1].w && iClickY >= rTypeButton[iType][1].y && iClickY < rTypeButton[iType][1].y + rTypeButton[iType][1].h) {
                                    ignoreclick = true;
...
enum {
    MAPHAZARD_EDIT_STATE_SELECT,
    MAPHAZARD_EDIT_STATE_TYPE,
    MAPHAZARD_EDIT_STATE_LOCATION,
    MAPHAZARD_EDIT_STATE_PROPERTIES
};
                ...
                if (event.button.button == SDL_BUTTON_LEFT && !ignoreclick) {
                    ...
                    if (MAPHAZARD_EDIT_STATE_SELECT == iEditState) {
                        ...
                        for (int iMapHazard = 0; iMapHazard < static_cast<int>(g_map->maphazards.size()); iMapHazard++) {
                            if (iClickX >= rIconRects[iMapHazard][1].x && iClickX < rIconRects[iMapHazard][1].x + rIconRects[iMapHazard][1].w && iClickY >= rIconRects[iMapHazard][1].y && iClickY < rIconRects[iMapHazard][1].y + rIconRects[iMapHazard][1].h) {
                        ...
                        if (g_map->maphazards.size() < MAXMAPHAZARDS && iClickX >= rNewButton[1].x && iClickX < rNewButton[1].x + rNewButton[1].w && iClickY >= rNewButton[1].y && iClickY < rNewButton[1].y + rNewButton[1].h) {
                            iEditMapHazard = NewMapHazard();
                            iEditState = MAPHAZARD_EDIT_STATE_TYPE;
                            ignoreclick = true;
                        }
                ...
                if (event.button.button == SDL_BUTTON_LEFT) {
                    ...
                    if (modeitemmode == 0) {
                        for (short iGoal = 0; iGoal < g_map->iNumRaceGoals; iGoal++) {
                            if (iMouseX >= g_map->racegoallocations[iGoal].x && iMouseX < g_map->racegoallocations[iGoal].x + 36 && iMouseY >= g_map->racegoallocations[iGoal].y && iMouseY < g_map->racegoallocations[iGoal].y + 36) {
                                dragmodeitem = iGoal;
                        ...
                        for (short iBase = 0; iBase < g_map->iNumFlagBases; iBase++) {
                            if (iMouseX >= g_map->flagbaselocations[iBase].x && iMouseX < g_map->flagbaselocations[iBase].x + 32 && iMouseY >= g_map->flagbaselocations[iBase].y && iMouseY < g_map->flagbaselocations[iBase].y + 32) {
                                dragmodeitem = iBase;
            ...
            case SDL_MOUSEBUTTONDOWN:
                if (event.button.button == SDL_BUTTON_LEFT || event.button.button == SDL_BUTTON_RIGHT) {
                    for (short iBackground = 0; iBackground < 16; iBackground++) {
                        ...
                        if (event.button.x >= rDst[iBackground].x && event.button.x < rDst[iBackground].x + rDst[iBackground].w && event.button.y >= rDst[iBackground].y && event.button.y < rDst[iBackground].y + rDst[iBackground].h) {
    ...
    for (int k = 0; k < MAPHEIGHT; k++) {
        for (int j = 0; j < MAPWIDTH; j++) {
            if (moveselectedtiles[j][k]) {
                if (j + movex >= 0 && j + movex < MAPWIDTH && k + movey >= 0 && k + movey < MAPHEIGHT) {
