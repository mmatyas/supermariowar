...
void MI_NetworkListScroll::Draw()
{
    ...
    for (short iLine = 0; iLine < iNumLines && (unsigned short)iLine < items.size(); iLine++) {
        ...
        rm->menu_font_large.drawChopRight(m_pos.x + 28, m_pos.y + 5 + iLine * 32 + 32, iWidth - 104, items[iOffset + iLine].sLeft);
        rm->menu_font_large.drawRightJustified(m_pos.x + 28 + iWidth - 64, m_pos.y + 5 + iLine * 32 + 32, items[iOffset + iLine].sRight);
    }
}
